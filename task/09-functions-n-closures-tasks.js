...
function getPolynom() {
  ...
  return x => {
    let out = 0;
    for (let i = 0; i < args.length; i++) out += args[i] * Math.pow(x, i);
    return out;
  }
}
...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${args.map((x) => JSON.stringify(x))}) starts`);
    let out = func(...args);
    logFunc(`${func.name}(${args.map((x) => JSON.stringify(x))}) ends`);
    return out;
  }
}
